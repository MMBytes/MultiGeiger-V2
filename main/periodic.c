...
// V2.5.18: the heap-guard auto-reboot lived here (V2.5.14 periodic_heap_guard)
// but sampled at ~1 Hz on the main task, so it caught the transient largest-
// free dip that any inbound HTTP connection causes — the monitor's own GET
// /log rebooted the Feather every ~4h. It now lives in transmission.c::
// tx_heap_guard(), evaluated once per TX cycle on the resting per-cycle heap
// snapshot. See transmission.c.
