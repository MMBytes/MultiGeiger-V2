...
**Review hardening (pre-tag, local MAX code review):** (1) `guard_removed` now tallies only the *marginal* edges (past the 190 µs gate) instead of every in-window edge, so it no longer double-credits the 50–190 µs afterpulses the gate already owned; (2) the edge-delta and inter-pulse-gap `uint32` casts are clamped to `UINT32_MAX`, closing a latent wrap (a >71.6 min quiet gap could otherwise mis-block a genuine pulse / mis-record min-max) — the clamp also covers the pre-existing count-path `dt`; (3) live-apply replaces the reboot-required handling (the guard is a software volatile, not a hardware latch); (4) the `edt` subtraction is computed once and reused by the guard; (5) the field became a checkbox+window pair (was a bare numeric, which mis-implied a dependency on PCNT) with the window range enforced at 200–20000.
