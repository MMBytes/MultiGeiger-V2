...
static void format_sgp41(char *out, size_t sz) {
    ...
    if (sgp41_get_nox_index(&nox_index) != ESP_OK) {
        ...
        snprintf(out, sz,
            ...
            "</div>",
            sgp41_had_valid_reading() ? "sensor unresponsive"
...
static esp_err_t config_get(httpd_req_t *req) {
    ...
    // V2.6.4: static — same justification as e_* + br_opts (V2.4.22).
    static char tube_opts[384];
    int tube_n = 0;
    ...
    int n = snprintf(body, CFG_FORM_BUF_SIZE,
                     ...
                     "<b>This REPLACES the always-on 190&micro;s dead-time gate</b> as the "
                     "protection on the published count &mdash; it is not an extra layer. The "
                     "gate rejects an edge for arriving too soon; this rejects it for being too "
                     "narrow. Which one your board needs depends on its phantom population: on "
                     "the ESP32-S3 the narrow pulses pass the gate, so this filter genuinely "
                     "lowers CPM, while on a board whose phantoms are narrow <i>and</i> "
                     ...
                     "id=\"dt_guard\" %s> "
                     "Dead-time guard &mdash; collapses 1-5ms re-trigger trains to one count "
                     "(time domain; reaches what the width filter can't). <b>Costly: at the "
                     "default 3000&micro;s window and background rates (~90 CPM), chance "
                     "coincidence alone discards roughly 4.5 genuine counts per 1000, and "
                     "proportionally more as the count rate rises. Its target population was "
                     "not found on either board tested 2026-08-29 &mdash; leave OFF unless "
                     "characterising.</b> <b>Mutually "
                     "exclusive with the PCNT width filter above</b> &mdash; if PCNT is on it "
                     "takes the count and this is forced off. <b>Cannot reach anything under "
                     "190&micro;s</b>: the window floor is 200&micro;s, and edges inside the "
                     "dead-time gate are already rejected by it, so they never reach this guard. "
                     "If you are chasing the 50-190&micro;s band on the <code>DIAG</code> line, "
                     "this is not the control &mdash; that band is a count-node RC recovery "
                     "artifact and the gate already excludes it.</label></div>"
                     ...
                     "pulse, ~10&micro;s after FET turn-off). <b>Only Rev B/C carrier layouts "
                     "have this defect</b> &mdash; on a Rev A or Rev B V2 board it catches "
                     "nothing and is a no-op. <b>Check before enabling:</b> with this box OFF, "
                     "compare <code>hv_coincident</code> and <code>hv_pulses</code> on the "
                     "<code>DIAG</code>/<code>CYCLE</code> lines. Tracking ~1:1 means your board "
                     "is affected; ~0.3%% of counts is the chance-coincidence floor and means it "
                     "is not. <b>Do this first</b> &mdash; once blanking is on, blanked edges "
                     "skip the coincidence tally and <code>hv_coincident</code> falls to that "
                     "same ~0.3%% floor whether or not you had the defect, so an affected node "
                     "becomes indistinguishable from a clean one. On an affected node it "
                     "<b>steps CPM down by the phantom share (~6-9%%)</b> &mdash; "
...
static esp_err_t config_post(httpd_req_t *req) {
    ...
    //
    // V2.6.4: static instead of stack — config_t is ~4 KB (dominated by
    // mqtt_tls_ca[2401]) and putting that on the 8 KB httpd stack caused a stack
    // overflow on POST (confirmed via coredump). Static is safe: httpd runs all
    // handlers serially on one task (see http_server_start), so config_post is
    // never re-entered. Every call reinitialises via cfg_next = *s_cfg before any
    // read. Follows the same pattern as config_get's e_* + br_opts (V2.4.22).
    static config_t cfg_next;
    cfg_next = *s_cfg;
    ...
    esp_err_t err = config_save(&cfg_next);
    if (err != ESP_OK) {
...
// that is written before either server starts, and uses no scratch buffer.
// log_access()/peer_ipstr() are stack-only (char ipstr[48] + getpeername), so
// the access line is safe on either task.
//
// A re-issue writes into tls_cert.c's own staging buffers, never into the
// live statics, so this route and the fingerprint on / keep showing exactly
// the certificate :443 is presenting until the deferred reboot swaps both at
// once — and no reader here can catch a half-written PEM.
#if HAL_HAS_HTTPS
...
/** @brief 400 with a plain-text body, suppressed on HEAD. httpd_resp_send_err
 *         has no HEAD special-casing, and the caller below is an HTTP_ANY
 *         route, so its body would be read as the next response by a
 *         keep-alive client. */
static esp_err_t send_400(httpd_req_t *req, const char *msg) {
    const bool head = (req->method == HTTP_HEAD);
    httpd_resp_set_status(req, "400 Bad Request");
    httpd_resp_set_type(req, "text/plain");
    return httpd_resp_send(req, head ? NULL : msg, head ? 0 : HTTPD_RESP_USE_STRLEN);
}
...
static esp_err_t redirect_to_https(httpd_req_t *req) {
    char host[64];
    if (httpd_req_get_hdr_value_str(req, "Host", host, sizeof(host)) != ESP_OK) {
        return send_400(req, "Host header missing or too long\n");
    }
    ...
    if (!https_redirect_location(host, req->uri, loc, sizeof(loc))) {
        return send_400(req, "unusable Host header\n");
    }
...
void http_server_start(config_t *cfg, const char *chip_id) {
    ...
    // httpd_config_t, so copy `hc` in wholesale — stack, handler count and LRU
    // purge all carry over (httpd_ssl_start overwrites server_port with
    // port_secure; the OTA recv timeout is overridden to 10 s below). The
