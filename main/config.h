 ...
 *
 *  V2.6.29: 0 when the `hv_blank` enable is clear, else `hv_blank_us` —
 *  deliberately NOT suppressed by `pcnt_filter` (unlike the guard): blanking
 *  COMPOSES with the width filter via the subtract mode (do_tx_cycle /
 *  history.c take pcnt count minus hv_blanked); see config_fields.def
 *  hv_blank and the pure blank_effective_us() (tube_logic.h) that pins this.
 *  Used by both the boot path (main.c) and the live /config apply
 *  (http_server.c).
 */
