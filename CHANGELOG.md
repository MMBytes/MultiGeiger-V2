...
**What:** New opt-in `deadtime_guard_us` config field (µs; `0` = off = default). A *retriggerable* refractory layered on top of the fixed 190 µs ISR dead-time gate: an edge can only start a new count after a quiet gap longer than the window; edges arriving inside it extend the dead zone and are dropped, collapsing an afterpulse / re-trigger **train** to a single count. The per-cycle count of suppressed edges is surfaced on the `DIAG:` line as `guard_removed=N`.
...
**How:** The GMC count ISR keys the window on the *previous edge* (`isr_last_edge_us`, captured before its per-edge update), which makes it retriggerable — every edge in a burst, counted or not, keeps the channel dead, so the whole train collapses to the leading count. `guard_removed` rides the same locked snapshot+reset as the raw-edge profiler (`tube_get_diag` gained one out-param). The config field auto-generates through the existing X-macro schema (struct / NVS / POST), with a `/config` numeric input (greyed via `syncTube()` and force-cleared when the tube is off) and a boot config-dump line. Set once at `tube_setup` (reboot-required, mirroring `pcnt_filter`). **Bench-validated on the QT Py** at `guard_removed=5000`: `guard_removed` exactly equals the sum of all sub-5 ms `edt_us` histogram bins, the cycle count drops ~6 % (matching the predicted spurious fraction), and the raw-edge histogram is unchanged — i.e. same pulse train, spurious 1–5 ms population no longer counted. Off by default on every board, so the release is behaviourally inert until a node opts in.
